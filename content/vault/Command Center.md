---
title: "Command Center"
---


*Advice for the day*
> STOP LARPING! You wanna get shit done? Scared of failure? DO IT SCARED ANYWAY!
## Daily Tasks
- [ ] DSA
	- [x] Greedy
		- [ ] 2 Medium
			- [ ] [Insert Interval - LeetCode](https://leetcode.com/problems/insert-interval/description/)
			- [ ] [Merge Intervals - LeetCode](https://leetcode.com/problems/merge-intervals/description/)
	- [x] BST 
		- [x] Theory
	- [ ] Revise Problems
 - [ ] Room
	- [ ] Arrange Wardrobe
	- [x] Wash Clothes
- [ ] Finish all of Machine Learning Theory + Make Notes
	- [x] Decision Trees
	- [ ] Boosting
	- [ ] Random Forests
	- [ ] RNN
	- [ ] LSTM 
	- [ ] Methods of Stability
		- [ ] Dropout
		- [ ] Regularisation
		- [ ] 
- [ ] AlphaZero Blog and Project Revision - Finish this
- [ ] Increase Linux partition space
- [x] Hosted my Obsidian Notes

# Weekly Goals
- [ ] Detailed Note on PCA and t-SNE
- [ ] BT Medium Problems
- [ ] DP - first 3 modules
- [ ] Revision of All modules
- [ ] 


- [ ] Retrain/modify and test AlphaZero
- [ ] Figure out vast.ai remote access stuff. check if antigravity compatible
	- [ ] Train the diffusion lm repo on larger dataset.

## Other Stuff
- [x] Improve the blog
	- [x] elaborate more on the confusing parts
	- [x] finish and upload the code
- [ ] Retrain/modify and test AlphaZero
- [ ] MIT CSAIL Flow and Diffusion
- [ ] Start reformulating Lossfunk PS
	- [ ] Read the old writeups and stuff
## Important Notes
- [Workout Log](/vault/misc/workout-log/)
- [[Project Ideas]]
- [Summer Plan 2026](/vault/misc/summer-plan-2026/)
